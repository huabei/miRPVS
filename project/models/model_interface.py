...
class MInterface(pl.LightningModule):
    ...
    def on_train_epoch_end(self) -> None:
        '''计算训练集的mae'''
        self.train_pred_y = torch.concat([x[:, 0] for x in self.training_step_outputs['preds']]).detach().cpu().numpy()
        self.train_true_y = torch.concat([x[:, 0] for x in self.training_step_outputs['true']]).detach().cpu().numpy()
    ...
    def on_validation_epoch_end(self) -> None:
        '''计算验证集的mae'''
        self.val_pred_y = torch.concat([x[:, 0] for x in self.validation_step_outputs['preds']]).detach().cpu().numpy()
        self.val_true_y = torch.concat([x[:, 0] for x in self.validation_step_outputs['true']]).detach().cpu().numpy()
    ...
    def on_test_epoch_end(self):
        ...
        self.test_pred_y = torch.concat([x[:, 0] for x in self.test_step_outputs['preds']]).detach().cpu().numpy()
        self.test_true_y = torch.concat([x[:, 0] for x in self.test_step_outputs['true']]).detach().cpu().numpy()
