...
class MInterface(pl.LightningModule):
    ...
    # def on_train_end(self) -> None:
        ...
        # self.logger.experiment.add_figure('train_fig', train_fig)
        # self.logger.experiment.add_figure('val_fig', val_fig)
    ...
    def validation_step(self, batch, batch_idx):
        # x, edge_index, edge_weight, t_energy = batch.x, batch.edge_index, batch.edge_attr, batch.y
        res = self(batch)
        # self.val_predictions['prediction'].extend(res.cpu().numpy())
        # self.val_predictions['true'].extend(batch.y.cpu().numpy())
        loss = self.loss_function(torch.squeeze(res), batch.y)
        ...
        self.log('val_acc', acc, on_step=False, on_epoch=True, prog_bar=True, batch_size=self.hparams.batch_size)
        return {'loss': loss}
    ...
    def test_step(self, batch, batch_idx):
        ...
        logits = self(batch)
        self.test_predictions['prediction'].append(logits)
        self.test_predictions['true'].append(batch.y)
        # loss = self.loss_function(torch.squeeze(logits), batch.y)
        return self.loss_function(torch.squeeze(logits), batch.y)

    def on_test_end(self):
        # Make the Progress Bar leave there
        # x = np.array(self.test_predictions['true'])
        x = torch.concat(self.test_predictions['true'], dim=0)
        ...
        # for logger in self.loggers:
        # self.logger.experiment.add_figure('test_fig', test_fig)
        self.logger.log_hyperparams(self.hparams, {'acc': acc})
        # self.logger.log_metrics({'test_r2': test_r2, 'train_r2': self.train_r2, 'val_r2': self.val_r2})
