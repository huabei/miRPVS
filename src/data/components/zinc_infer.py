...
# 保存日志到文件
logging.basicConfig(level=logging.INFO, filename='zinc_infer_complete.log', filemode='a', format='%(asctime)s - %(pathname)s[line:%(lineno)d] - %(levelname)s: %(message)s')
...
class ZincInferDataset(InMemoryDataset):
    ...
    def __init__(self, data_dir, dataset, transform=None, pre_transform=None, pre_filter=None):
        ...
        super().__init__(None, transform=transform, pre_transform=pre_transform, pre_filter=pre_filter)
