...
class MoleculeGnn(Module):
    ...
    def __init__(
        ...
        self.m_gcn = ModuleList([MoleculeGCN(in_node_nf, hidden_nf) for _ in range(n_layers)])

        self.lin = ModuleList([Linear(hidden_nf, hidden_nf) for _ in range(out_layers)])
