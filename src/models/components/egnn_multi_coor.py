...
class E_GCL(nn.Module):
    """E(n) Equivariant Convolutional Layer re."""
    ...
    def __init__(
        ...
        self.tanh = tanh
        self.epsilon = 1e-8
        edge_coords_nf = coor_channels  # edge length
        ...
        # 产生coord的权重
        layer = nn.Linear(hidden_nf, coor_channels, bias=False)  # 产生多个channel的coord权重
    ...
    def edge_model(self, source, target, radial):
        # 产生edge的特征.
        out = torch.cat([source, target, radial.squeeze(1)], dim=1)
    ...
    def node_model(self, x, edge_index, edge_feat, node_attr):
        row, col = edge_index
        agg = unsorted_segment_sum_edge(edge_feat, row, num_segments=x.size(0))
    ...
    def coord_model(self, coord, edge_index, coord_diff, edge_feat):
        row, col = edge_index
        trans = coord_diff * self.coord_mlp(edge_feat).unsqueeze(1)  # (E, 3, c)
        ...
        coord = coord + agg
        return coord  # (N, 3, c) 更新后的坐标

    def coord2radial(self, edge_index, coord):
        row, col = edge_index
        coord_diff = coord[row] - coord[col]  # 相对位置 (E, 3, c)
        radial = torch.sum(coord_diff**2, 1, keepdim=True)  # 半径 (E, 1, c)
        # print(radial.shape)
        if self.normalize:
            norm = torch.sqrt(radial).detach() + self.epsilon
            # print(norm.shape, coord_diff.shape)

            coord_diff = coord_diff / norm

        return radial, coord_diff  # 半径和相对位置

    def forward(self, h, edge_index, coord, node_attr=None):  # coord:(N, 3, 1)
        row, col = edge_index
        radial, coord_diff = self.coord2radial(edge_index, coord)  # 计算半径和相对位置

        edge_feat = self.edge_model(h[row], h[col], radial)  # 产生edge的特征:(E, hidden_nf)
        ...
        return h, coord, edge_feat  # 新的node特征和坐标


class EgnnMultiCoor(nn.Module):
    def __init__(
        self,
        in_node_nf,
        hidden_nf,
        ...
        **kwargs
        ...
        self.coor_channels = coor_channels
    ...
    def forward_(self, batch):
        ...
        x = batch.pos
        # 为x添加channel维度
        x = x.unsqueeze(-1).expand(-1, -1, self.coor_channels)
...
def unsorted_segment_sum_edge(data, segment_ids, num_segments):
