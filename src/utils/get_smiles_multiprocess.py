...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', filename='get_smiles.log')
...
if not os.path.exists('total_zinc_id_set.pt'):
    ...
    for folder in folders:
        ...
        datas.append(set(torch.load(folder+'_top_data.pt')[:,0]))
    # 进行并集操作
    total_zinc_id = set()
    for data in datas:
