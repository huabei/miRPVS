...
class ZincPdbqt:
    """A class for pdbqt or pdbqt.gz file, this class could transfer str dict to some friendly
    format."""
...
def ele_filter(zinc_pdbqt_item, elements_list=None):
    """if pdbqt item have element that not in elements_list, return False, else return True. Use in
    filter() function.
...
def read_dock_score(origin_data_path: str) -> dict:
    ...
    """
    file_list = [
        os.path.join(origin_data_path, i)
        for i in os.listdir(origin_data_path)
        if i.endswith(".pkl")
    ]
    score_dict = dict()
...
def statistic_pocket_interaction(pocket_atom):
    """statistic 17-20 letters by length in every line start with ATOM in file pocket_atom output
    like:
...
def get_pocket_info(pocket_folder):
    ...
    for i in range(pocket_num):
        i += 1
        pocket_dict[i] = get_pdb_atom_info(os.path.join(pocket_folder, f"pocket{i}_vert.pqr"))[
            "ATOM"
        ]
    return pocket_dict
...
def get_pdb_atom_info(file_path):
    ...
    for row in f:
        if row[:6] in ["HETATM", "ATOM  "]:
            ...
            atom_info_dict[row[:6].strip()].append(
                (atom_info["atom_name"], atom_info["x"], atom_info["y"], atom_info["z"])
            )
    f.close()
