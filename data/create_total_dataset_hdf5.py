    ...
    with gzip.open(pdbqt_gz_path, 'rb') as f:
        ...
        for line in f:
            if line.startswith(b'ATOM'):
                coor.append([str(line[12:14].strip(), 'utf-8'),
            ...
            for path, sub_dir, filename in tqdm(os.walk(data_folder)):
                for file in filename:
                    if file.endswith('.pdbqt.gz'):
                        ...
                        d = os.path.join(path.replace(data_folder, os.path.basename(
                            data_folder)), '_'.join(file.split('.')[:2])).replace('-', '_')
                        # 分析pdbqt.gz文件,加入try except是因为有些文件可能有问题
                        try:
                            logging.info(
                                f'processing {os.path.join(path, file)}')
                            store_coor[d], store_index[d] = analyze_zinc_pdbqt_gz(
                                os.path.join(path, file))
                        except:
                            logging.warning(
                                f'error in {os.path.join(path, file)}')
                            continue
